...
void generic_tape_device::OpenDevice(DeviceControlRecord* dcr, DeviceMode omode)
{
   ...
   * Windows Code
   */
  if ((fd_ = d_open(archive_device, oflags, 0)) < 0) { dev_errno = errno; }
#else
  /*
   * UNIX Code
   *
  ...
  for (;;) {
     ...
     */
    fd_ = d_open(archive_device, oflags | O_NONBLOCK, 0);
    if (fd_ < 0) {
      BErrNo be;
      dev_errno = errno;
      ...
      if (d_ioctl(fd_, MTIOCTOP, (char*)&mt_com) < 0) {
         ...
         */
        d_close(fd_);
        fd_ = d_open(archive_device, oflags, 0); /* open normally */
        if (fd_ < 0) {
          BErrNo be;
          dev_errno = errno;
          Dmsg5(100, "Open error on %s omode=%d oflags=%x errno=%d: ERR=%s\n",
...
void generic_tape_device::LockDoor()
{
  ...
  mt_com.mt_op = MTLOCK;
  mt_com.mt_count = 1;
  if (d_ioctl(fd_, MTIOCTOP, (char*)&mt_com) < 0) { clrerror(mt_com.mt_op); }
#endif
}
...
void generic_tape_device::SetOsDeviceParameters(DeviceControlRecord* dcr)
{
  ...
  if (bstrcmp(dev->archive_device, "/dev/null")) {
...
int32_t generic_tape_device::GetOsTapeFile()
{
  ...
  if (HasCap(CAP_MTIOCGET) && d_ioctl(fd_, MTIOCGET, (char*)&mt_stat) == 0) {
...
char* generic_tape_device::StatusDev()
{
  ...
  if (d_ioctl(fd_, MTIOCGET, (char*)&mt_stat) < 0) {
    BErrNo be;
