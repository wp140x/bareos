...
static ResourceItem dev_items[] = {
  ...
  {"ArchiveDevice", CFG_TYPE_STRNAME, ITEM(res_dev, archive_device), 0, CFG_ITEM_REQUIRED, NULL, NULL, NULL},
    ...
    if (d && d->dev_type == DeviceType::B_DROPLET_DEV) {
      if (d->max_concurrent_jobs == 0) {
        ...
        Jmsg2(nullptr, M_ERROR_TERM, 0,
              ...
              d->archive_device, d->max_concurrent_jobs);
      }
    }
  }
}
...
static void FreeResource(BareosResource* res, int type)
{
  ...
  switch (type) {
    ...
    case R_DEVICE: {
      ...
      if (p->archive_device) { free(p->archive_device); }
